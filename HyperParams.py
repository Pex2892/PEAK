...
from sklearn.model_selection import train_test_split, KFold, RepeatedKFold, GridSearchCV, LeaveOneOut
...
class HyperParams:
    ...
    def best_split(self, df, colsX, colsY, max_splits=5, max_repeats=10, seed:int = 2021, filename: str = 'K-fold_TrainTestSplit.csv'):
        ...
        for cols in combs:
            for k in range(2, max_splits):
                to_test.append([list(cols), RepeatedKFold(n_splits=k, n_repeats=max_repeats, random_state=seed)])
